...
public final class RatPoly {
    ...
    public RatPoly(RatTerm rt) {
        ...
        terms = new ArrayList<>();
        //By default, all elements of ArrayList when initiated are zero. so rf.isZero() is 0 already when the array is created.
    ...
    public RatPoly(int c, int e) {
        // TODO: Fill in this method, then remove the RuntimeException
        terms = new ArrayList<>();
        // if c == 0, then 'terms' is 0 already by default.
        if (!(c == 0)) {
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        ...
        // Multiplying the Coefficients by a scalar of 0 will result in 0. So clear the list.
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        for (int i = 0; i < lst.size(); i++) {
            // case 1: the exponent is bigger than the following exponent in descending order
            if (newTerm.getExpt() > lst.get(i).getExpt()) {
                lst.add(i, newTerm);
                return;
            } else if (newTerm.getExpt() == lst.get(i).getExpt()) {
                //case 2: the exponent is equal to the term in the lst
        ...
        //case 3: the exponent is the smallest
        lst.add(newTerm);
    ...
    public RatPoly negate() {
        ...
        if (this.isNaN()) {
            ...
            List<RatTerm> list = new ArrayList<>(terms);
    ...
    public RatPoly add(RatPoly p) {
        ...
        if (this.isNaN() || p.isNaN()) {
            ...
            List<RatTerm> list = new ArrayList<>(p.terms);
            for (RatTerm rt: terms) {
                sortedInsert(list, rt);
            }
            return new RatPoly(list);
        }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        List<RatTerm> list = new ArrayList<>();
        int j = 0;
    ...
    public RatPoly div(RatPoly p) {
        ...
        if (!this.equals(ZERO)) {
            ...
            // while degree of this is greater than divisor
            while (remain.degree() >= p.degree() && remain.terms.size() != 0) {
